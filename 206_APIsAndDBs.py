...
try:
	...
	CACHE_DICTION = {'timeline_info':{}, 'user_info':{}}
...
def get_user_tweets(user):
	if user in CACHE_DICTION['timeline_info']: 
		...
		print ('retreving data from internet') 
		t_line = api.user_timeline(user, count = 20) 
		CACHE_DICTION['timeline_info'][user] = t_line 
...
for tweet in umich_tweets:
	for users in tweet['entities']['user_mentions']:
		if users['id'] not in m_users:
			m_users.append((users['id']))
...
for user in m_users:
	if user not in CACHE_DICTION['user_info']:
		...
		cache_file.write(json.dumps(cache_diction, indent = 4))
		cache_file.close()
	u_info = cache_diction['user_info'][users]
	t_info = (u_info['id_str'], u_info['screen_name'], u_info['favourites_count'], u_info['description'])
	cur.execute('INSERT INTO Users (user_id, screen_name, num_favs, description) VALUES (?,?,?,?)', t_info) 
...
for kevin in umich_tweets:
	t_info = (kevin['id_str'], kevin['text'], kevin['user']['id_str'], kevin['created_at'], kevin['retweet_count'])
	cur.execute('INSERT INTO Tweets (tweet_id, text, user_posted, time_posted, retweets) VALUES (?,?,?,?,?)', t_info)
conn.commit()
...
users_info = [kevin for kevin in cur]
...
screen_names = [kevin[0] for kevin in cur]


# Make a query to select all of the tweets (full rows of tweet information)
# that have been retweeted more than 10 times. Save the result 
# (a list of tuples, or an empty list) in a variable called retweets.
cur.execute('SELECT tweet_id, text, user_posted, time_posted, retweets FROM Tweets WHERE retweets > 10')

retweets = [kevin for kevin in cur] 
...
favorites = [kevin[0] for kevin in cur]
...
joined_data = [kevin for kevin in cur]
# Make a query using an INNER JOIN to get a list of tuples with 2 
# elements in each tuple: the user screenname and the text of the 
